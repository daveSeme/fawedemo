...
class Review_narrative_reports extends \App\Controllers\BaseController
{
    ...
    public function __construct()
    {
        ...
        check_permision("review_narrative_reports", "1", 0);
    }
    public function index()
    {
        ...
        $data["title"] = "Review Narrative Reports";
        $data["main_title"] = "Review/Approve";
        $data["base_id"] = $this->session->get("office");
        $model = new \App\Models\review_approve\Review_project_quarterly_narrative_report_model();
        $data["data"] = $model->where("base_id", $data["base_id"])->orderBy("id", "DESC")->findAll();
        $data["js_file"] = "office/review_approve/review_narrative_reports/js_file";
        $data["main_content"] = "office/review_approve/review_narrative_reports/list";
        echo view("template/index", $data);
    }
    public function review($id = NULL)
    {
        check_permision("review_narrative_reports", "4", 0);
        $data["user_id"] = $this->tank_auth->get_user_id();
        $data["username"] = $this->tank_auth->get_username();
        $data["user_level"] = $this->tank_auth->get_user_level();
        $data["title"] = "Review Narrative Reports";
        ...
        if ($this->request->getMethod() === "post") {
            ...
            if ($this->validate->withRequest($this->request)->run()) {
                ...
                $model = new \App\Models\review_approve\Approve_project_quarterly_narrative_report_model();
                $model->update($id, $postdata);
                $post_comments_data = ["workflow_id" => $id, "base_id" => $data["base_id"], "report_type" => "Quarterly Narrative Report", "user_type" => $this->session->get("user_type"), "reviwer_id" => $data["user_id"], "reviwer_comments" => $this->request->getVar("reviwer_comments"), "review_date" => date("Y-m-d h:m:s"), "report_status" => $this->request->getVar("report_status"), "createdby" => $data["user_id"], "createdtime" => date("Y-m-d h:m:s")];
                ...
                if ($this->request->getVar("report_status") == 0) {
                    ...
                    $data["subject"] = "Quarterly Narrative Report Approval Notification";
                    $data["record_id"] = $id;
                    $users_data = get_by_id("id", $submitted_by, "ctbl_users");
                    $data["email"] = $users_data["email"];
                    $this->_send_email("project_quarterly_narrative_report_approve", $data["email"], $data, $data["subject"]);
                    $this->session->setFlashdata("feedback", "Report Approved successfully.");
                    return $this->response->redirect(site_url("review_approve/review_narrative_reports"));
                }
                if ($this->request->getVar("report_status") == 3) {
                    $submitted_by = $data["stdata"]["submitted_by"];
                    $data["site_name"] = $this->configTankAuth->website_name;
                    $data["subject"] = "Quarterly Narrative Report Rejection Notification";
                    $data["record_id"] = $id;
                    $users_data = get_by_id("id", $submitted_by, "ctbl_users");
                    $data["email"] = $users_data["email"];
                    $this->_send_email("project_quarterly_narrative_report_reject", $data["email"], $data, $data["subject"]);
        ...
        $data["js_file"] = "office/review_approve/review_narrative_reports/js_file";
        $data["main_content"] = "office/review_approve/review_narrative_reports/review";
        echo view("template/index", $data);
    }
    public function view($id = NULL)
    {
        ...
        $data["username"] = $this->tank_auth->get_username();
        $data["user_level"] = $this->tank_auth->get_user_level();
        $data["title"] = "View Narrative Reports";
        $data["main_title"] = "Review/Approve";
        $data["pid"] = $id;
        $model = new \App\Models\review_approve\Review_project_quarterly_narrative_report_model();
        $data["stdata"] = $model->where("id", $id)->first();
        $data["js_file"] = "office/review_approve/review_narrative_reports/js_file";
        $data["main_content"] = "office/review_approve/review_narrative_reports/view";
        echo view("template/index", $data);
    }
    public function download_excel()
    {
        ...
        ob_start();
        echo view("office/review_approve/review_narrative_reports/export", $data);
        $table = ob_get_clean();
