...
class Cases_field_office extends \App\Controllers\BaseController
{
    ...
    public function __construct()
    {
        ...
        check_permision("cases_field_office", "1", 0);
    }
    public function index()
    {
        ...
        $data["title"] = "Cases Database Report";
        $data["main_title"] = "Access System Reports / Cases Database Reports";
        $data["base_id"] = $this->session->get("office");
        $data["year"] = "";
        if ($this->request->getMethod() === "post") {
            $data["year"] = $this->request->getVar("year");
            $data["month"] = $this->request->getVar("month");
            $data["county"] = $this->request->getVar("county");
        }
        $data["js_file"] = "office/system_reports/cases_report/cases_field_office/js_file";
        $data["main_content"] = "office/system_reports/cases_report/cases_field_office/list";
    ...
    public function download_excel()
    {
        ...
        $data["title"] = "Cases Database Report";
        $data["year"] = $this->request->uri->getSegment(4);
        $data["month"] = $this->request->uri->getSegment(5);
        $data["county"] = $this->request->uri->getSegment(6);
        $data["filename"] = $data["title"] . "_" . date("d_m_Y_h_i_a") . ".xls";
        ob_start();
        echo view("office/system_reports/cases_report/cases_field_office/export", $data);
        $table = ob_get_clean();
