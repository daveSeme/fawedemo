...
class subexecuting_partner extends \App\Controllers\BaseController
{
    ...
    public function __construct()
    {
        ...
        if ($this->session->get("user_type") == "admin") {
            check_permision("subexecuting_partner", "1", 1);
        } else {
            check_permision("subexecuting_partner", "1", 0);
        }
    }
    public function index()
    {
        ...
        $data["title"] = "SubExecuting Partner";
        $data["main_title"] = "System Configuration";
        $data["base_id"] = $this->session->get("office");
        $Model = new \App\Models\master\Subexecuting_partner_Model();
        $data["data"] = $Model->orderBy("id", "DESC")->findAll();
        $data["js_file"] = "office/master/subexecuting_partner/js_file";
        $data["main_content"] = "office/master/subexecuting_partner/list";
    ...
    public function add()
    {
        ...
        $data["username"] = $this->tank_auth->get_username();
        $data["user_level"] = $this->tank_auth->get_user_level();
        $data["title"] = "Add SubExecuting Partner";
        $data["main_title"] = "System Configuration";
        $data["base_id"] = $this->session->get("office");
        if ($this->session->get("user_type") == "admin") {
            check_permision("subexecuting_partner", "1", 1);
        ...
        if ($this->request->getMethod() === "post") {
            ...
            if ($this->validate->withRequest($this->request)->run()) {
                ...
                trail($status, "insert", "subexecuting_partner", $data_post);
                $this->session->setFlashdata("feedback", "Record created successfully.");
                if ($this->request->getVar("action") == "save") {
                    return $this->response->redirect(site_url("master/subexecuting_partner/add"));
                }
                return $this->response->redirect(site_url("master/subexecuting_partner"));
            }
            $data["validator"] = $this->validator;
        }
        $data["js_file"] = "office/master/subexecuting_partner/add_js";
        $data["main_content"] = "office/master/subexecuting_partner/add";
        echo view("template/index", $data);
    }
    public function delete($id = NULL)
    {
        ...
        $Model = new \App\Models\master\Subexecuting_partner_Model();
        $where = $id;
        $previous_values = get_by_id("id", $id, "subexecuting_partner");
        $status = $Model->where("id", $id)->delete($id);
        trail($status, "delete", "subexecuting_partner", $where, $previous_values);
    ...
    public function edit($id = NULL)
    {
        ...
        $data["username"] = $this->tank_auth->get_username();
        $data["user_level"] = $this->tank_auth->get_user_level();
        $data["title"] = "Edit SubExecuting Partner";
        $data["main_title"] = "System Configuration";
        $data["base_id"] = $this->session->get("office");
        $model = new \App\Models\master\Subexecuting_partner_Model();
        ...
        $data["js_file"] = "office/master/subexecuting_partner/add_js";
        $data["main_content"] = "office/master/subexecuting_partner/edit";
        echo view("template/index", $data);
    }
}
