...
class workshop_reporting_tool extends \App\Controllers\BaseController
{
    ...
    public function index() {
        ...
        $Model = new \App\Models\knowledge_center\Workshop_reporting_tool_model();
        $data["data"] = $Model->orderBy("id", "DESC")->findAll();
        $data["js_file"] = "office/knowledge_center/workshop_reporting_tool/js_file";
        $data["main_content"] = "office/knowledge_center/workshop_reporting_tool/list";
        echo view("template/index", $data);
    }

    public function add()
    {
        check_permision("workshop_reporting_tool", "2", 0);
        $data["user_id"] = $this->tank_auth->get_user_id();
        $data["username"] = $this->tank_auth->get_username();
        $data["user_level"] = $this->tank_auth->get_user_level();
        $data["title"] = "Add Workshop Reporting Tool";
        $data["main_title"] = "Knowledge Center";
        $data["base_id"] = $this->session->get("office");
        if ($this->request->getMethod() === "post") {
            $this->validate->setRules(["project" => ["label" => "Project", "rules" => "required|trim"]]);
            $data["errors"] = [];
            // $query = $this->db->query("SELECT * FROM workshop_reporting_tool where base_id=\"" . $data["base_id"] . "\" and project=\"" . $this->request->getVar("project") . "\"  ");
            ...
            if ($this->validate->withRequest($this->request)->run()) {
                    ...
                    if ($imagefile = $this->request->getFiles()) {
                        foreach ($imagefile["file"] as $img) {
                            if ($img->isValid() && !$img->hasMoved()) {
                                ...
                                $this->db->table("workshop_reporting_tool_documents")->insert($data_to_store_other);
                            }
                        }
                    }
                ...
                if(is_array($target)) {
                    foreach ($target as $key => $v) {
                        ...
                        $this->db->table("workshop_reporting_targets")->insert($post_target_data);
                    }
                }
                ...
                if ($this->request->getVar("action") == "submit_report") {
                    ...
                    return $this->response->redirect(site_url("knowledge_center/workshop_reporting_tool"));
                } else {
        ...
        $data["js_file"] = "office/knowledge_center/workshop_reporting_tool/add_js";
        $data["main_content"] = "office/knowledge_center/workshop_reporting_tool/add";
        echo view("template/index", $data);
    }
    public function edit($id = NULL)
    {
        check_permision("workshop_reporting_tool", "4", 0);
        $data["user_id"] = $this->tank_auth->get_user_id();
        $data["username"] = $this->tank_auth->get_username();
        $data["user_level"] = $this->tank_auth->get_user_level();
        $data["title"] = "Edit Workshop Reporting Tool";
        $data["main_title"] = "Knowledge Center";
        $data["base_id"] = $this->session->get("office");
        $model = new \App\Models\knowledge_center\Workshop_reporting_tool_model();
        ...
        if ($this->request->getMethod() === "post") {
            ...
            if ($this->validate->withRequest($this->request)->run()) {
                ...
                if ($imagefile = $this->request->getFiles()) {
                    foreach ($imagefile["file"] as $img) {
                        if ($img->isValid() && !$img->hasMoved()) {
                            $newName = $img->getRandomName();
                            $title = $img->getName();
                            $img->move(ROOTPATH . "public/uploads/workshop_reporting_tool", $newName);
                ...
                if(is_array($target)) {
                    ...
                    $this->db->table("workshop_reporting_targets")->delete(["workflow_id" => $id,"project_id" => $this->request->getVar("project")]);
                    foreach ($target as $key => $v) {
                ...
                if ($this->request->getVar("action") == "submit_report") {
                    ...
                    return $this->response->redirect(site_url("knowledge_center/activity_reporting_tool"));
                }
            } else {
                $data["validator"] = $this->validator;
                echo "failed";
        ...
        $data["js_file"] = "office/knowledge_center/workshop_reporting_tool/add_js";
        $data["main_content"] = "office/knowledge_center/workshop_reporting_tool/edit";
        echo view("template/index", $data);
    }
    ...
    {
        check_permision("workshop_reporting_tool", "3", 0);
        $model = new \App\Models\knowledge_center\Workshop_reporting_tool_model();
        $previous_values = $model->where("id", $id)->first();
        $where = $id;
        trail(1, "delete", "Workshop Report", $where, $previous_values);
    ...
    public function view($id = NULL)
    {
        ...
        $data["username"] = $this->tank_auth->get_username();
        $data["user_level"] = $this->tank_auth->get_user_level();
        $data["title"] = "View Workshop Reporting Tool";
        $data["main_title"] = "Knowledge Center";
        $data["pid"] = $id;
        $model = new \App\Models\knowledge_center\Workshop_reporting_tool_model();
        $data["stdata"] = $model->where("id", $id)->first();
        $data["js_file"] = "office/knowledge_center/workshop_reporting_tool/js_file";
        $data["main_content"] = "office/knowledge_center/workshop_reporting_tool/view";
        echo view("template/index", $data);
    }
    ...
    public function download_excel()
    {
        ...
        $data["title"] = "Workshop Reporting Tool";
        $model = new \App\Models\knowledge_center\Workshop_reporting_tool_model();
        $data["stdata"] = $model->where("id", $data["id"])->first();
        $data["filename"] = $data["title"] . "_" . date("d_m_Y_h_i_a") . ".xls";
        ob_start();
        echo view("office/knowledge_center/workshop_reporting_tool/export", $data);
        $table = ob_get_clean();
    ...
    public function delete_docs($id = NULL)
    {
        ...
        $this->db->table("workshop_reporting_tool_documents")->delete(["id" => $id]);
        $this->session->setFlashdata("feedback", "Record deleted successfully.");
        return $this->response->redirect(site_url("knowledge_center/workshop_reporting_tool/edit/" . $workflow_id));
    }
}
