...
class Org_annual_indicator_tracking_report extends \App\Controllers\BaseController
{
    ...
    public function __construct()
    {
        ...
        check_permision("org_annual_indicator_tracking_report", "1", 0);
    }
    public function index()
    {
        ...
        $model = new \App\Models\reporting\organizational_data\Org_annual_indicator_tracking_report_model();
        $data["data"] = $model->where("base_id", $data["base_id"])->orderBy("id", "DESC")->findAll();
        $data["js_file"] = "office/reporting/organizational_data/org_annual_indicator_tracking_report/js_file";
        $data["main_content"] = "office/reporting/organizational_data/org_annual_indicator_tracking_report/list";
        echo view("template/index", $data);
    }
    public function add()
    {
        check_permision("org_annual_indicator_tracking_report", "2", 0);
        $data["user_id"] = $this->tank_auth->get_user_id();
        $data["username"] = $this->tank_auth->get_username();
        $data["user_level"] = $this->tank_auth->get_user_level();
        ...
        if ($this->request->getMethod() === "post") {
            $this->validate->setRules(["strategic_plan" => ["label" => "Strategic Plan", "rules" => "required|trim"], "year" => ["label" => "Year", "rules" => "required|trim"], "report_name" => ["label" => "Report Name", "rules" => "required|trim"]]);
            $data["errors"] = [];
            $query = $this->db->query("SELECT * FROM org_annual_indicator_tracking_report where base_id=\"" . $data["base_id"] . "\" and strategic_plan=\"" . $this->request->getVar("strategic_plan") . "\" and  year = \"" . $this->request->getVar("year") . "\" ");
            $results = $query->getResult();
            if (count($results) <= 0) {
                if ($this->validate->withRequest($this->request)->run()) {
                    $postdata = ["strategic_plan" => $this->request->getVar("strategic_plan"), "year" => $this->request->getVar("year"), "base_id" => $this->session->get("office"), "report_name" => $this->request->getVar("report_name"), "createdby" => $data["user_id"], "createtime" => date("Y-m-d H:i:s")];
                    $model = new \App\Models\reporting\organizational_data\Org_annual_indicator_tracking_report_model();
                        ...
                        if (is_array($indicator_id)) {
                            foreach ($indicator_id as $key => $n) {
                                if ($indicator_id != "") {
                                    ...
                                    $this->db->table("org_annual_indicator_tracking_report_map")->insert($data_to_store_other);
                                }
                            }
                        }
                    }
                    ...
                    if ($this->request->getVar("action") == "save") {
                        return $this->response->redirect(site_url("reporting/organizational_data/org_annual_indicator_tracking_report/add"));
                    }
                    return $this->response->redirect(site_url("reporting/organizational_data/org_annual_indicator_tracking_report"));
                } else {
        ...
        $data["js_file"] = "office/reporting/organizational_data/org_annual_indicator_tracking_report/add_js";
        $data["main_content"] = "office/reporting/organizational_data/org_annual_indicator_tracking_report/add";
        echo view("template/index", $data);
    }
    public function edit($id = NULL)
    {
        check_permision("org_annual_indicator_tracking_report", "4", 0);
        $data["user_id"] = $this->tank_auth->get_user_id();
        $data["username"] = $this->tank_auth->get_username();
        $data["user_level"] = $this->tank_auth->get_user_level();
        ...
        if ($this->request->getMethod() === "post") {
            ...
            if ($this->validate->withRequest($this->request)->run()) {
                ...
                $query_delete = $this->db->query("delete from org_annual_indicator_tracking_report_map  where  workflow_id =\"" . $id . "\"");
                $category = $this->request->getVar("category");
                $indicator_id = $this->request->getVar("indicator_id");
                $unit = $this->request->getVar("unit");
                $target = $this->request->getVar("target");
        ...
        $data["js_file"] = "office/reporting/organizational_data/org_annual_indicator_tracking_report/add_js";
        $data["main_content"] = "office/reporting/organizational_data/org_annual_indicator_tracking_report/edit";
        echo view("template/index", $data);
    }
    public function delete($id = NULL)
    {
        check_permision("org_annual_indicator_tracking_report", "3", 0);
        $model = new \App\Models\reporting\organizational_data\Org_annual_indicator_tracking_report_model();
    ...
    public function view($id = NULL)
    {
        ...
        $data["stdata"] = $model->where("id", $id)->first();
        $data["js_file"] = "office/reporting/organizational_data/org_annual_indicator_tracking_report/js_file";
        $data["main_content"] = "office/reporting/organizational_data/org_annual_indicator_tracking_report/view";
        echo view("template/index", $data);
    }
    ...
    public function download_excel()
    {
        ...
        $data["title"] = "Org Annual Indicator Tracking Report";
        $model = new \App\Models\reporting\organizational_data\Org_annual_indicator_tracking_report_model();
        $data["stdata"] = $model->where("id", $data["id"])->first();
        $data["filename"] = $data["title"] . "_" . date("d_m_Y_h_i_a") . ".xls";
        ob_start();
        echo view("office/reporting/organizational_data/org_annual_indicator_tracking_report/export", $data);
        $table = ob_get_clean();
