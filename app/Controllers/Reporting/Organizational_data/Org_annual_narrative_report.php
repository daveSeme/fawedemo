...
class org_annual_narrative_report extends \App\Controllers\BaseController
{
    ...
    public function __construct()
    {
        ...
        check_permision("org_annual_narrative_report", "1", 0);
    }
    public function index()
    {
        ...
        $model = new \App\Models\reporting\organizational_data\Org_annual_narrative_report_model();
        $data["data"] = $model->where("base_id", $data["base_id"])->orderBy("id", "DESC")->findAll();
        $data["js_file"] = "office/reporting/organizational_data/org_annual_narrative_report/js_file";
        $data["main_content"] = "office/reporting/organizational_data/org_annual_narrative_report/list";
        echo view("template/index", $data);
    }
    public function add()
    {
        check_permision("org_annual_narrative_report", "2", 0);
        $data["user_id"] = $this->tank_auth->get_user_id();
        $data["username"] = $this->tank_auth->get_username();
        $data["user_level"] = $this->tank_auth->get_user_level();
        ...
        if ($this->request->getMethod() === "post") {
            $this->validate->setRules(["strategic_plan" => ["label" => "Strategic Plan", "rules" => "required|trim"], "year" => ["label" => "Year", "rules" => "required|trim"], "key_highlights" => ["label" => "key highlights Section", "rules" => "required|trim"], "challenges_experienced" => ["label" => "Challenges experienced and Mitigating measure", "rules" => "required|trim"], "success_stories" => ["label" => "Success Stories", "rules" => "required|trim"], "activities_anticipated" => ["label" => "Activities Anticipated for Next Reporting Period", "rules" => "required|trim"]]);
            $data["errors"] = [];
            $query = $this->db->query("select * FROM org_annual_narrative_report where base_id=\"" . $data["base_id"] . "\" and strategic_plan=\"" . $this->request->getVar("strategic_plan") . "\" and  year = \"" . $this->request->getVar("year") . "\" ");
            $results = $query->getResult();
            if (count($results) <= 0) {
                if ($this->validate->withRequest($this->request)->run()) {
                    $postdata = ["base_id" => $this->session->get("office"), "strategic_plan" => $this->request->getVar("strategic_plan"), "year" => $this->request->getVar("year"), "key_highlights" => $this->request->getVar("key_highlights"), "challenges_experienced" => $this->request->getVar("challenges_experienced"), "success_stories" => $this->request->getVar("success_stories"), "activities_anticipated" => $this->request->getVar("activities_anticipated"), "createdby" => $data["user_id"], "createtime" => date("Y-m-d H:i:s")];
                    $model = new \App\Models\reporting\organizational_data\Org_annual_narrative_report_model();
                    ...
                    if ($workflow_id != "" && ($imagefile = $this->request->getFiles())) {
                        foreach ($imagefile["file"] as $img) {
                            if ($img->isValid() && !$img->hasMoved()) {
                                ...
                                $title = $img->getName();
                                $img->move(ROOTPATH . "public/uploads/org_annual_narrative_report", $newName);
                    ...
                    if ($this->request->getVar("action") == "save") {
                        return $this->response->redirect(site_url("reporting/organizational_data/org_annual_narrative_report/add"));
                    }
                    return $this->response->redirect(site_url("reporting/organizational_data/org_annual_narrative_report/"));
                } else {
        ...
        $data["js_file"] = "office/reporting/organizational_data/org_annual_narrative_report/add_js";
        $data["main_content"] = "office/reporting/organizational_data/org_annual_narrative_report/add";
        echo view("template/index", $data);
    }
    public function edit($id = NULL)
    {
        check_permision("org_annual_narrative_report", "4", 0);
        $data["user_id"] = $this->tank_auth->get_user_id();
        $data["username"] = $this->tank_auth->get_username();
        $data["user_level"] = $this->tank_auth->get_user_level();
        ...
        if ($this->request->getMethod() === "post") {
            ...
            if ($this->validate->withRequest($this->request)->run()) {
                ...
                $postdata = ["strategic_plan" => $this->request->getVar("strategic_plan"), "year" => $this->request->getVar("year"), "key_highlights" => $this->request->getVar("key_highlights"), "challenges_experienced" => $this->request->getVar("challenges_experienced"), "success_stories" => $this->request->getVar("success_stories"), "activities_anticipated" => $this->request->getVar("activities_anticipated"), "updatedby" => $data["user_id"], "updatedtime" => date("Y-m-d H:i:s")];
                $model = new \App\Models\reporting\organizational_data\Org_annual_narrative_report_model();
        ...
        $data["js_file"] = "office/reporting/organizational_data/org_annual_narrative_report/add_js";
        $data["main_content"] = "office/reporting/organizational_data/org_annual_narrative_report/edit";
        echo view("template/index", $data);
    }
    public function delete($id = NULL)
    {
        check_permision("org_annual_narrative_report", "3", 0);
        $model = new \App\Models\reporting\organizational_data\Org_annual_narrative_report_model();
    ...
    public function view($id = NULL)
    {
        ...
        $data["stdata"] = $model->where("id", $id)->first();
        $data["js_file"] = "office/reporting/organizational_data/org_annual_narrative_report/js_file";
        $data["main_content"] = "office/reporting/organizational_data/org_annual_narrative_report/view";
        echo view("template/index", $data);
    }
    ...
    public function download_excel()
    {
        ...
        $data["title"] = "Org Annual Narrative Report";
        $model = new \App\Models\reporting\organizational_data\Org_annual_narrative_report_model();
        $data["stdata"] = $model->where("id", $data["id"])->first();
        $data["filename"] = $data["title"] . "_" . date("d_m_Y_h_i_a") . ".xls";
        ob_start();
        echo view("office/reporting/organizational_data/org_annual_narrative_report/export", $data);
        $table = ob_get_clean();
    ...
    public function delete_docs($id = NULL)
    {
        ...
        $this->session->setFlashdata("feedback", "Record deleted successfully.");
        return $this->response->redirect(site_url("reporting/organizational_data/org_annual_narrative_report/edit/" . $workflow_id));
    }
}
