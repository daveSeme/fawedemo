...
class project_quarterly_indicator_tracking_report extends \App\Controllers\BaseController
{
    ...
    public function __construct()
    {
        ...
        check_permision("project_quarterly_indicator_tracking_report", "1", 0);
    }
    public function index()
    {
        ...
        $model = new \App\Models\reporting\project_data\Project_quarterly_indicator_tracking_report_model();
        $data["data"] = $model->where("base_id", $data["base_id"])->orderBy("id", "DESC")->findAll();
        $data["js_file"] = "office/reporting/project_data/project_quarterly_indicator_tracking_report/js_file";
        $data["main_content"] = "office/reporting/project_data/project_quarterly_indicator_tracking_report/list";
        echo view("template/index", $data);
    }
    public function add()
    {
        check_permision("project_quarterly_indicator_tracking_report", "2", 0);
        $data["user_id"] = $this->tank_auth->get_user_id();
        $data["username"] = $this->tank_auth->get_username();
        $data["user_level"] = $this->tank_auth->get_user_level();
        ...
        if ($this->request->getMethod() === "post") {
            ...
            $data["errors"] = [];
            $query = $this->db->query("SELECT * FROM project_quarterly_indicator_tracking_report where base_id=\"" . $data["base_id"] . "\" and project=\"" . $this->request->getVar("project") . "\" and  year = \"" . $this->request->getVar("year") . "\" and  quarter = \"" . $this->request->getVar("quarter") . "\"   ");
            $results = $query->getResult();
            if (count($results) <= 0) {
                if ($this->validate->withRequest($this->request)->run()) {
                    $postdata = ["project" => $this->request->getVar("project"), "year" => $this->request->getVar("year"), "quarter" => $this->request->getVar("quarter"), "base_id" => $this->session->get("office"), "report_name" => $this->request->getVar("report_name"), "report_status" => "1", "createdby" => $data["user_id"], "createtime" => date("Y-m-d H:i:s")];
                    $model = new \App\Models\reporting\project_data\Project_quarterly_indicator_tracking_report_model();
                        ...
                        if (is_array($indicator_id)) {
                            foreach ($indicator_id as $key => $n) {
                                if ($indicator_id != "") {
                                    ...
                                    $this->db->table("project_quarterly_indicator_tracking_report_map")->insert($data_to_store_other);
                                }
                            }
                        }
                    }
                    if ($this->request->getVar("action") == "submit_report") {
                        ...
                        $model = new \App\Models\reporting\project_data\Project_quarterly_indicator_tracking_report_submit_model();
                        $model->update($workflow_id, $post_report_data);
                        $data["site_name"] = $this->configTankAuth->website_name;
                        $data["subject"] = "Indicator Tracking Report Submitted for Review/Approve";
                        ...
                        if (is_array($email_to)) {
                            foreach ($email_to as $to_value) {
                                ...
                                $this->_send_email("project_quarterly_it_submit", $data["email"], $data, $data["subject"]);
                            }
                        }
                        $this->session->setFlashdata("feedback", "Report Submitted successfully.");
                        return $this->response->redirect(site_url("reporting/project_data/project_quarterly_indicator_tracking_report"));
                    } else {
                ...
                $this->session->setFlashdata("feedback", "Indicator Tracking Report already created for this Project year and quarter, Please enter new one.");
            }
        }
        $data["js_file"] = "office/reporting/project_data/project_quarterly_indicator_tracking_report/add_js";
        $data["main_content"] = "office/reporting/project_data/project_quarterly_indicator_tracking_report/add";
        echo view("template/index", $data);
    }
    public function edit($id = NULL)
    {
        check_permision("project_quarterly_indicator_tracking_report", "4", 0);
        $data["user_id"] = $this->tank_auth->get_user_id();
        $data["username"] = $this->tank_auth->get_username();
        $data["user_level"] = $this->tank_auth->get_user_level();
        ...
        if ($this->request->getMethod() === "post") {
            ...
            if ($this->validate->withRequest($this->request)->run()) {
                ...
                $query_delete = $this->db->query("delete from project_quarterly_indicator_tracking_report_map  where  workflow_id =\"" . $id . "\"");
                $category = $this->request->getVar("category");
                $indicator_id = $this->request->getVar("indicator_id");
                $unit = $this->request->getVar("unit");
                $target = $this->request->getVar("target");
        ...
        $data["js_file"] = "office/reporting/project_data/project_quarterly_indicator_tracking_report/add_js";
        $data["main_content"] = "office/reporting/project_data/project_quarterly_indicator_tracking_report/edit";
        echo view("template/index", $data);
    }
    public function delete($id = NULL)
    {
        check_permision("project_quarterly_indicator_tracking_report", "3", 0);
        $model = new \App\Models\reporting\project_data\Project_quarterly_indicator_tracking_report_model();
    ...
    public function view($id = NULL)
    {
        ...
        $data["stdata"] = $model->where("id", $id)->first();
        $data["js_file"] = "office/reporting/project_data/project_quarterly_indicator_tracking_report/js_file";
        $data["main_content"] = "office/reporting/project_data/project_quarterly_indicator_tracking_report/view";
        echo view("template/index", $data);
    }
    ...
    public function download_excel()
    {
        ...
        ob_start();
        echo view("office/reporting/project_data/project_quarterly_indicator_tracking_report/export", $data);
        $table = ob_get_clean();
