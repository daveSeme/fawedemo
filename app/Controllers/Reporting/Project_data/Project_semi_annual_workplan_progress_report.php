...
class Project_semi_annual_workplan_progress_report extends \App\Controllers\BaseController
{
    ...
    public function __construct()
    {
        ...
        check_permision("project_semi_annual_workplan_progress_report", "1", 0);
    }
    public function index()
    {
        ...
        $data["title"] = "Project Semi-Annual Workplan Progress Report";
        $data["main_title"] = "Project Data / Reporting";
        $data["base_id"] = $this->session->get("office");
        $model = new \App\Models\reporting\project_data\Project_semi_annual_workplan_progress_report_model();
        $data["data"] = $model->where("base_id", $data["base_id"])->orderBy("id", "DESC")->findAll();
        $data["js_file"] = "office/reporting/project_data/project_semi_annual_workplan_progress_report/list_js";
        $data["main_content"] = "office/reporting/project_data/project_semi_annual_workplan_progress_report/list";
        echo view("template/index", $data);
    }
    public function add()
    {
        check_permision("project_semi_annual_workplan_progress_report", "2", 0);
        $data["user_id"] = $this->tank_auth->get_user_id();
        $data["username"] = $this->tank_auth->get_username();
        $data["user_level"] = $this->tank_auth->get_user_level();
        $data["title"] = "Add Project Semi-Annual Workplan Progress Report";
        $data["main_title"] = "Project Data / Reporting";
        $data["base_id"] = $this->session->get("office");
        if ($this->request->getMethod() === "post") {
            ...
            $data["errors"] = [];
            $query = $this->db->query("SELECT * FROM project_semi_annual_workplan_progress_report where base_id=\"" . $data["base_id"] . "\" and project=\"" . $this->request->getVar("project") . "\" and  year = \"" . $this->request->getVar("year") . "\" and  quarter = \"" . $this->request->getVar("quarter") . "\"   ");
            ...
            if (count($results) <= 0) {
                if ($this->validate->withRequest($this->request)->run()) {
                        ...
                        if (is_array($activity_id)) {
                            foreach ($activity_id as $key => $n) {
                                if ($activity_id != "") {
                                    ...
                                    $this->db->table("project_semi_annual_workplan_progress_report_mapping")->insert($data_to_store_other);
                                }
                            }
                        }
                    }
                    ...
                    if ($this->request->getVar("action") == "save") {
                        return $this->response->redirect(site_url("reporting/project_data/project_semi_annual_workplan_progress_report/add"));
                    }
                    return $this->response->redirect(site_url("reporting/project_data/project_semi_annual_workplan_progress_report/"));
                } else {
                    $data["validator"] = $this->validator;
                    echo "failed";
                }
            } else {
                $this->session->setFlashdata("feedback", "Semi-Annual Workplan Progress Report already created for this Project year and quarter, Please enter new one.");
            }
        }
        $data["js_file"] = "office/reporting/project_data/project_semi_annual_workplan_progress_report/js_file";
        $data["main_content"] = "office/reporting/project_data/project_semi_annual_workplan_progress_report/add";
        echo view("template/index", $data);
    }
    public function edit($id = NULL)
    {
        check_permision("project_semi_annual_workplan_progress_report", "4", 0);
        $data["user_id"] = $this->tank_auth->get_user_id();
        $data["username"] = $this->tank_auth->get_username();
        $data["user_level"] = $this->tank_auth->get_user_level();
        $data["title"] = "Edit Semi-Annual Workplan Progress Report";
        $data["main_title"] = "Project Data / Reporting";
        $data["base_id"] = $this->session->get("office");
        $model = new \App\Models\reporting\project_data\Project_semi_annual_workplan_progress_report_model();
        ...
        if ($this->request->getMethod() === "post") {
            ...
            if ($this->validate->withRequest($this->request)->run()) {
                ...
                $query_delete = $this->db->query("DELETE FROM project_semi_annual_workplan_progress_report_mapping  where  workflow_id =\"" . $id . "\"");
                $activity_id = $this->request->getVar("activity_id");
                $status = $this->request->getVar("status");
                $budget = $this->request->getVar("budget");
                $budget = $this->request->getVar("budget");
        ...
        $data["js_file"] = "office/reporting/project_data/project_semi_annual_workplan_progress_report/list_js";
        $data["main_content"] = "office/reporting/project_data/project_semi_annual_workplan_progress_report/edit";
        echo view("template/index", $data);
    }
    public function delete($id = NULL)
    {
        check_permision("project_semi_annual_workplan_progress_report", "3", 0);
        $model = new \App\Models\reporting\project_data\Project_semi_annual_workplan_progress_report_model();
        $previous_values = $model->where("id", $id)->first();
        $where = $id;
        trail(1, "delete", "Project Semi-Annual Workplan Progress Report", $where, $previous_values);
    ...
    public function view($id = NULL)
    {
        ...
        $data["username"] = $this->tank_auth->get_username();
        $data["user_level"] = $this->tank_auth->get_user_level();
        $data["title"] = "View Project Semi-Annual Workplan Progress Report";
        $data["main_title"] = "Project Data / Reporting";
        $data["pid"] = $id;
        $model = new \App\Models\reporting\project_data\Project_semi_annual_workplan_progress_report_model();
        $data["stdata"] = $model->where("id", $id)->first();
        $data["js_file"] = "office/reporting/project_data/project_semi_annual_workplan_progress_report/list_js";
        $data["main_content"] = "office/reporting/project_data/project_semi_annual_workplan_progress_report/view";
        echo view("template/index", $data);
    }
    ...
    public function get_activity_by_year()
    {
        ...
        foreach ($results_mon_progress_report as $row_mon_progress_report) {
            $query = $this->db->query("SELECT sum(budget) as q_budget from project_activity_annual_plan Where activity_id=\"" . $row_mon_progress_report["activity_id"] . "\" and year = \"" . $data["year"] . "\" and  quarter IN (" . $data["quarter"] . ") ");
            $row = $query->getRowArray();
            echo "                    <tr>\r\n                    <td width=\"50%\">\r\n                    ";
    ...
    public function download_excel()
    {
        ...
        ob_start();
        echo view("office/reporting/project_data/project_semi_annual_workplan_progress_report/export", $data);
        $table = ob_get_clean();
