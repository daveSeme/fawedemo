...
class Project_semi_annual_narrative_report extends \App\Controllers\BaseController
{
    ...
    public function __construct()
    {
        ...
        check_permision("project_semi_annual_narrative_report", "1", 0);
    }
    public function index()
    {
        ...
        $data["js_file"] = "office/reporting/project_data/project_semi_annual_narrative_report/js_file";
        $data["main_content"] = "office/reporting/project_data/project_semi_annual_narrative_report/list";
        echo view("template/index", $data);
    }
    public function add()
    {
        check_permision("project_semi_annual_narrative_report", "2", 0);
        $data["user_id"] = $this->tank_auth->get_user_id();
        $data["username"] = $this->tank_auth->get_username();
        $data["user_level"] = $this->tank_auth->get_user_level();
        ...
        if ($this->request->getMethod() === "post") {
            ...
            $data["errors"] = [];
            $query = $this->db->query("SELECT * FROM project_semi_annual_narrative_report where base_id=\"" . $data["base_id"] . "\" and project=\"" . $this->request->getVar("project") . "\" and  year = \"" . $this->request->getVar("year") . "\" and  quarter = \"" . $this->request->getVar("quarter") . "\"   ");
            ...
            if (count($results) <= 0) {
                if ($this->validate->withRequest($this->request)->run()) {
                    ...
                    if ($workflow_id != "" && ($imagefile = $this->request->getFiles())) {
                        foreach ($imagefile["file"] as $img) {
                            if ($img->isValid() && !$img->hasMoved()) {
                                ...
                                $title = $img->getName();
                                $img->move(ROOTPATH . "public/uploads/project_semi_annual_narrative_report", $newName);
                                $data_to_store_other = ["workflow_id" => $workflow_id, "base_id" => $this->session->get("office"), "type" => "Semi-Annual Report", "title" => $title, "documents" => $newName];
                    ...
                    if ($this->request->getVar("action") == "save") {
                        return $this->response->redirect(site_url("reporting/project_data/project_semi_annual_narrative_report/add"));
                    }
                    return $this->response->redirect(site_url("reporting/project_data/project_semi_annual_narrative_report/"));
                } else {
        ...
        $data["js_file"] = "office/reporting/project_data/project_semi_annual_narrative_report/add_js";
        $data["main_content"] = "office/reporting/project_data/project_semi_annual_narrative_report/add";
        echo view("template/index", $data);
    }
    public function edit($id = NULL)
    {
        check_permision("project_semi_annual_narrative_report", "4", 0);
        $data["user_id"] = $this->tank_auth->get_user_id();
        $data["username"] = $this->tank_auth->get_username();
        $data["user_level"] = $this->tank_auth->get_user_level();
                ...
                if ($imagefile = $this->request->getFiles()) {
                    foreach ($imagefile["file"] as $img) {
                        if ($img->isValid() && !$img->hasMoved()) {
                            ...
                            $title = $img->getName();
                            $img->move(ROOTPATH . "public/uploads/project_semi_annual_narrative_report", $newName);
                            $data_to_store_other = ["workflow_id" => $id, "base_id" => $this->session->get("office"), "type" => "Semi-Annual Report", "title" => $title, "documents" => $newName];
                            $this->db->table("project_narrative_report_documents")->insert($data_to_store_other);
                        }
                    }
                }
        ...
        $data["js_file"] = "office/reporting/project_data/project_semi_annual_narrative_report/add_js";
        $data["main_content"] = "office/reporting/project_data/project_semi_annual_narrative_report/edit";
        echo view("template/index", $data);
    }
    public function delete($id = NULL)
    {
        check_permision("project_semi_annual_narrative_report", "3", 0);
        $model = new \App\Models\reporting\project_data\Project_semi_annual_narrative_report_model();
    ...
    public function view($id = NULL)
    {
        ...
        $data["stdata"] = $model->where("id", $id)->first();
        $data["js_file"] = "office/reporting/project_data/project_semi_annual_narrative_report/js_file";
        $data["main_content"] = "office/reporting/project_data/project_semi_annual_narrative_report/view";
        echo view("template/index", $data);
    }
    ...
    public function download_excel()
    {
        ...
        $data["title"] = "Project Semi-Annual Narrative Report";
        $model = new \App\Models\reporting\project_data\Project_semi_annual_narrative_report_model();
        $data["stdata"] = $model->where("id", $data["id"])->first();
        $data["filename"] = $data["title"] . "_" . date("d_m_Y_h_i_a") . ".xls";
        ob_start();
        echo view("office/reporting/project_data/project_semi_annual_narrative_report/export", $data);
        $table = ob_get_clean();
    ...
    public function delete_docs($id = NULL)
    {
        ...
        $this->session->setFlashdata("feedback", "Record deleted successfully.");
        return $this->response->redirect(site_url("reporting/project_data/project_semi_annual_narrative_report/edit/" . $workflow_id));
    }
}
