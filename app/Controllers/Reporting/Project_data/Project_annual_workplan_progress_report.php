...
class Project_annual_workplan_progress_report extends \App\Controllers\BaseController
{
    ...
    public function __construct()
    {
        ...
        check_permision("project_annual_workplan_progress_report", "1", 0);
    }
    public function index()
    {
        ...
        $data["title"] = "Project Annual Workplan Progress Report";
        $data["main_title"] = "Project Data / Reporting";
        $data["base_id"] = $this->session->get("office");
        $model = new \App\Models\reporting\project_data\Project_annual_workplan_progress_report_model();
        $data["data"] = $model->where("base_id", $data["base_id"])->orderBy("id", "DESC")->findAll();
        $data["js_file"] = "office/reporting/project_data/project_annual_workplan_progress_report/list_js";
        $data["main_content"] = "office/reporting/project_data/project_annual_workplan_progress_report/list";
        echo view("template/index", $data);
    }
    public function add()
    {
        check_permision("project_annual_workplan_progress_report", "2", 0);
        $data["user_id"] = $this->tank_auth->get_user_id();
        $data["username"] = $this->tank_auth->get_username();
        $data["user_level"] = $this->tank_auth->get_user_level();
        $data["title"] = "Add Project Annual Workplan Progress Report";
        $data["main_title"] = "Project Data / Reporting";
        $data["base_id"] = $this->session->get("office");
        if ($this->request->getMethod() === "post") {
            $this->validate->setRules(["project" => ["label" => "project Name", "rules" => "required|trim"], "year" => ["label" => "Year", "rules" => "required|trim"], "report_name" => ["label" => "Report Name", "rules" => "required"]]);
            $data["errors"] = [];
            $query = $this->db->query("SELECT * FROM project_annual_workplan_progress_report where base_id=\"" . $data["base_id"] . "\" and project=\"" . $this->request->getVar("project") . "\" and  year = \"" . $this->request->getVar("year") . "\" ");
            $results = $query->getResult();
            if (count($results) <= 0) {
                if ($this->validate->withRequest($this->request)->run()) {
                    $postdata = ["year" => $this->request->getVar("year"), "base_id" => $this->session->get("office"), "project" => $this->request->getVar("project"), "report_name" => $this->request->getVar("report_name"), "createdby" => $data["user_id"], "createtime" => date("Y-m-d H:i:s")];
                    $model = new \App\Models\reporting\project_data\Project_annual_workplan_progress_report_model();
                    ...
                    if ($workflow_id != "") {
                        ...
                        if (is_array($activity_id)) {
                            foreach ($activity_id as $key => $n) {
                                if ($activity_id != "") {
                                    $data_to_store_other = ["workflow_id" => $workflow_id, "base_id" => $this->session->get("office"), "year" => $map_year[$key], "activity_id" => $n, "status" => isset($status[$key]) ?: 0, "budget" => $budget[$key], "expenses" => $expenses[$key], "comments" => $comments[$key]];
                                    $this->db->table("project_annual_workplan_progress_report_mapping")->insert($data_to_store_other);
                                }
                            }
                        }
                    }
                    ...
                    if ($this->request->getVar("action") == "save") {
                        return $this->response->redirect(site_url("reporting/project_data/project_annual_workplan_progress_report/add"));
                    }
                    return $this->response->redirect(site_url("reporting/project_data/project_annual_workplan_progress_report/"));
                } else {
                    $data["validator"] = $this->validator;
                    echo "failed";
                }
            } else {
                $this->session->setFlashdata("feedback", "Quarterly Workplan Progress Report already created for this Project year, Please enter new one.");
            }
        }
        $data["js_file"] = "office/reporting/project_data/project_annual_workplan_progress_report/js_file";
        $data["main_content"] = "office/reporting/project_data/project_annual_workplan_progress_report/add";
        echo view("template/index", $data);
    }
    public function edit($id = NULL)
    {
        check_permision("project_annual_workplan_progress_report", "4", 0);
        $data["user_id"] = $this->tank_auth->get_user_id();
        $data["username"] = $this->tank_auth->get_username();
        $data["user_level"] = $this->tank_auth->get_user_level();
        ...
        if ($this->request->getMethod() === "post") {
            $this->validate->setRules(["year" => ["label" => "Year", "rules" => "required|trim"], "report_name" => ["label" => "Report Name", "rules" => "required|trim"], "project" => ["label" => "project Name", "rules" => "required"]]);
            $data["errors"] = [];
            if ($this->validate->withRequest($this->request)->run()) {
                ...
                $query_delete = $this->db->query("DELETE FROM project_annual_workplan_progress_report_mapping  where  workflow_id =\"" . $id . "\"");
                $activity_id = $this->request->getVar("activity_id");
                $status = $this->request->getVar("status");
                $budget = $this->request->getVar("budget");
                $budget = $this->request->getVar("budget");
                ...
                if (is_array($activity_id)) {
                    foreach ($activity_id as $key => $n) {
                        if ($activity_id != "") {
                            $data_to_store_other = ["workflow_id" => $id, "base_id" => $this->session->get("office"), "year" => $map_year[$key], "activity_id" => $n, "status" => isset($status[$key]) ?: 0, "budget" => $budget[$key], "expenses" => $expenses[$key], "comments" => $comments[$key]];
                            $this->db->table("project_annual_workplan_progress_report_mapping")->insert($data_to_store_other);
                        }
                    }
                }
        ...
        $data["js_file"] = "office/reporting/project_data/project_annual_workplan_progress_report/list_js";
        $data["main_content"] = "office/reporting/project_data/project_annual_workplan_progress_report/edit";
        echo view("template/index", $data);
    }
    public function delete($id = NULL)
    {
        check_permision("project_annual_workplan_progress_report", "3", 0);
        $model = new \App\Models\reporting\project_data\Project_annual_workplan_progress_report_model();
        $previous_values = $model->where("id", $id)->first();
        $where = $id;
        trail(1, "delete", "Project Annual Workplan Progress Report", $where, $previous_values);
    ...
    public function view($id = NULL)
    {
        ...
        $data["username"] = $this->tank_auth->get_username();
        $data["user_level"] = $this->tank_auth->get_user_level();
        $data["title"] = "View Project Annual Workplan Progress Report";
        $data["main_title"] = "Project Data / Reporting";
        $data["pid"] = $id;
        $model = new \App\Models\reporting\project_data\Project_annual_workplan_progress_report_model();
        $data["stdata"] = $model->where("id", $id)->first();
        $data["js_file"] = "office/reporting/project_data/project_annual_workplan_progress_report/list_js";
        $data["main_content"] = "office/reporting/project_data/project_annual_workplan_progress_report/view";
        echo view("template/index", $data);
    }
    ...
    public function get_activity_by_year()
    {
        ...
        foreach ($results_mon_progress_report as $row_mon_progress_report) {
            $query = $this->db->query("SELECT annul_budget as tot_budget FROM project_activity_annual_budget_map Where activity_id=\"" . $row_mon_progress_report["activity_id"] . "\" and year=\"" . $data["year"] . "\"  ");
            $row = $query->getRowArray();
            echo "                    <tr>\r\n                    <td width=\"50%\">\r\n                    ";
    ...
    public function download_excel()
    {
        ...
        ob_start();
        echo view("office/reporting/project_data/project_annual_workplan_progress_report/export", $data);
        $table = ob_get_clean();
