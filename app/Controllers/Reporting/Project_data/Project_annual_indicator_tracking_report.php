...
class Project_annual_indicator_tracking_report extends \App\Controllers\BaseController
{
    ...
    public function __construct()
    {
        ...
        check_permision("project_annual_indicator_tracking_report", "1", 0);
    }
    public function index()
    {
        ...
        $model = new \App\Models\reporting\project_data\Project_annual_indicator_tracking_report_model();
        $data["data"] = $model->where("base_id", $data["base_id"])->orderBy("id", "DESC")->findAll();
        $data["js_file"] = "office/reporting/project_data/project_annual_indicator_tracking_report/js_file";
        $data["main_content"] = "office/reporting/project_data/project_annual_indicator_tracking_report/list";
        echo view("template/index", $data);
    }
    public function add()
    {
        check_permision("project_annual_indicator_tracking_report", "2", 0);
        $data["user_id"] = $this->tank_auth->get_user_id();
        $data["username"] = $this->tank_auth->get_username();
        $data["user_level"] = $this->tank_auth->get_user_level();
        ...
        if ($this->request->getMethod() === "post") {
            $this->validate->setRules(["project" => ["label" => "Project", "rules" => "required|trim"], "year" => ["label" => "Year", "rules" => "required|trim"], "report_name" => ["label" => "Report Name", "rules" => "required|trim"]]);
            $data["errors"] = [];
            $query = $this->db->query("SELECT * FROM project_annual_indicator_tracking_report where base_id=\"" . $data["base_id"] . "\" and project=\"" . $this->request->getVar("project") . "\" and  year = \"" . $this->request->getVar("year") . "\"   ");
            $results = $query->getResult();
            if (count($results) <= 0) {
                if ($this->validate->withRequest($this->request)->run()) {
                    $postdata = ["project" => $this->request->getVar("project"), "year" => $this->request->getVar("year"), "base_id" => $this->session->get("office"), "report_name" => $this->request->getVar("report_name"), "createdby" => $data["user_id"], "createtime" => date("Y-m-d H:i:s")];
                    $model = new \App\Models\reporting\project_data\Project_annual_indicator_tracking_report_model();
                        ...
                        if (is_array($indicator_id)) {
                            foreach ($indicator_id as $key => $n) {
                                if ($indicator_id != "") {
                                    ...
                                    $this->db->table("project_annual_indicator_tracking_report_map")->insert($data_to_store_other);
                                }
                            }
                        }
                    }
                    ...
                    if ($this->request->getVar("action") == "save") {
                        return $this->response->redirect(site_url("reporting/project_data/project_annual_indicator_tracking_report/add"));
                    }
                    return $this->response->redirect(site_url("reporting/project_data/project_annual_indicator_tracking_report"));
                } else {
                    $data["validator"] = $this->validator;
                    echo "failed";
                }
            } else {
                $this->session->setFlashdata("feedback", "Indicator Tracking Report already created for this Project year, Please enter new one.");
            }
        }
        $data["js_file"] = "office/reporting/project_data/project_annual_indicator_tracking_report/add_js";
        $data["main_content"] = "office/reporting/project_data/project_annual_indicator_tracking_report/add";
        echo view("template/index", $data);
    }
    public function edit($id = NULL)
    {
        check_permision("project_annual_indicator_tracking_report", "4", 0);
        $data["user_id"] = $this->tank_auth->get_user_id();
        $data["username"] = $this->tank_auth->get_username();
        $data["user_level"] = $this->tank_auth->get_user_level();
        ...
        if ($this->request->getMethod() === "post") {
            ...
            if ($this->validate->withRequest($this->request)->run()) {
                ...
                $query_delete = $this->db->query("delete from project_annual_indicator_tracking_report_map  where  workflow_id =\"" . $id . "\"");
                $category = $this->request->getVar("category");
                $indicator_id = $this->request->getVar("indicator_id");
                $unit = $this->request->getVar("unit");
                $target = $this->request->getVar("target");
        ...
        $data["js_file"] = "office/reporting/project_data/project_annual_indicator_tracking_report/add_js";
        $data["main_content"] = "office/reporting/project_data/project_annual_indicator_tracking_report/edit";
        echo view("template/index", $data);
    }
    public function delete($id = NULL)
    {
        check_permision("project_annual_indicator_tracking_report", "3", 0);
        $model = new \App\Models\reporting\project_data\Project_annual_indicator_tracking_report_model();
    ...
    public function view($id = NULL)
    {
        ...
        $data["stdata"] = $model->where("id", $id)->first();
        $data["js_file"] = "office/reporting/project_data/project_annual_indicator_tracking_report/js_file";
        $data["main_content"] = "office/reporting/project_data/project_annual_indicator_tracking_report/view";
        echo view("template/index", $data);
    }
    ...
    public function download_excel()
    {
        ...
        $data["title"] = "Project Annual Indicator Tracking Report";
        $model = new \App\Models\reporting\project_data\Project_annual_indicator_tracking_report_model();
        $data["stdata"] = $model->where("id", $data["id"])->first();
        $data["filename"] = $data["title"] . "_" . date("d_m_Y_h_i_a") . ".xls";
        ob_start();
        echo view("office/reporting/project_data/project_annual_indicator_tracking_report/export", $data);
        $table = ob_get_clean();
