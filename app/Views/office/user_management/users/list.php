...
echo base_url() . "/user_management/users/add";
...
if ($data) {
    ...
    foreach ($data as $record) {
        ...
        echo "</td>\r\n\t\t\t\t\t\t\t\t\t\t\t\t  <td>";
        if ($record["user_type"] == "Implementing Partner") {
            $base_id = get_by_id("id", $record["base_id"], "implementing_partner");
            echo $base_id["name"];
        }
        echo "</td>\r\n\t\t\t\t\t\t\t\t\t\t\t\t  <td>";
        $user_group_id = get_by_id("id", $record["user_group_id"], "ctbl_usergroups");
        echo $user_group_id["title"];
        echo "</td>\r\n\t\t\t\t\t\t\t\t\t\t\t\t   <td>";
        echo $record["email"];
        echo "</td>\r\n\t\t\t\t\t\t\t\t\t\t\t\t    <td>";
        echo $record["contact_number"];
        echo "</td>\r\n\t\t\t\t\t\t\t\t\t\t\t\t\t  <td>\r\n                                                    <div class=\"d-flex demo\"> \r\n                                                       \r\n\t\t\t\t\t\t\t\t\t\t\t\t\t\t\r\n <a href=\"#\"  onClick=\"javascript:del_rec_1('";
        echo base_url("user_management/users/delete/" . $record["id"]);
        ...
        echo base_url("user_management/users/edit/" . $record["id"]);
