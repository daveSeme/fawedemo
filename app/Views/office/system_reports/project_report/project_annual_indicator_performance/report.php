...
$query_plan = $db->query("select * from project_annual_indicator_tracking_report where project = '" . $project . "' ");
...
if (!empty($results_plan)) {
    ...
    echo "    \r\n    \r\n    ";
    $query_mon_progress_report = $db->query("select * from project_annual_indicator_tracking_report_map where workflow_id = '" . $row_plan->id . "' and category = 'Outcome Indicator' order by indicator_id");
    $results_mon_progress_report = $query_mon_progress_report->getResultArray();
    foreach ($results_mon_progress_report as $row_mon_progress_report) {
        $unit_data = get_by_id("id", $row_mon_progress_report["unit"], "mas_unit");
        $unit_name = $unit_data["name"];
    ...
    echo "    \r\n    \r\n    ";
    $query_mon_progress_report = $db->query("select * from project_annual_indicator_tracking_report_map where workflow_id = '" . $row_plan->id . "' and category = 'Output Indicator' order by indicator_id");
    $results_mon_progress_report = $query_mon_progress_report->getResultArray();
    foreach ($results_mon_progress_report as $row_mon_progress_report) {
        $unit_data = get_by_id("id", $row_mon_progress_report["unit"], "mas_unit");
        $unit_name = $unit_data["name"];
